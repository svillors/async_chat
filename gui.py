...
async def draw(messages_queue, sending_queue, status_updates_queue):
    ...
    async with anyio.create_task_group() as task:
        task.start_soon(update_tk, root_frame)
        task.start_soon(update_conversation_history, conversation_panel, messages_queue)
        task.start_soon(update_status_panel, status_labels, status_updates_queue)
